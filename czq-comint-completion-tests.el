                    ...
                    (let ((sent-command nil)
                      ...
                      (cl-letf (((symbol-function 'comint-send-string)
                                 (lambda (_proc command)
                                   (setq sent-command (if sent-command
                                                         (concat sent-command command)
                                                       command))))
                      ...
                      (should-not (string-match-p "unwind-protect" sent-command))
                      (should-not (string-match-p "base64" sent-command))
                      (should (string-match-p "printf '%s\\\\n'" sent-command))
                      (let* ((restore-tag "<czq-comint handler=elisp>(setq czq-comint-output-enabled t)</czq-comint>")
                             (quoted-restore (shell-quote-argument restore-tag)))
                        (should (string-match-p (regexp-quote quoted-restore) sent-command)))
                      ...
                      (let ((process-messages '())
                            (pattern "<czq-comint handler=elisp>\\(\\(?:.\\|\n\\)*?\\)</czq-comint>")
                            (start 0))
                        (while (string-match pattern sent-command start)
                          (let* ((next-start (match-end 0))
                                 (raw (match-string 1 sent-command))
                                 (body (if (string-match "%s" raw)
                                           (format raw temp-dir)
                                         raw)))
                            ...
                            (cl-letf (((symbol-function 'message)
                                       (lambda (fmt &rest args)
                                         (push (apply #'format fmt args)
                                               process-messages))))
                              (czq-comint--handle-elisp body nil))))
                        (setq process-messages (nreverse process-messages))
                        ...
                        (should (cl-some (lambda (msg)
                                         ...
                                         process-messages)))
                      (czq-comint--handle-elisp "(setq czq-comint-output-enabled t)" nil)
                      (should czq-comint-output-enabled)))
